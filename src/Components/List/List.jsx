...
function List() {
    ...
    const list_items = result.map(item => {
      return <ListItem key={item.id} id={item.id} type = {type} overview={item.overview} release = {type === "movie" ? item.release_date : item.first_air_date} title = {type === "movie" ? item.title : item.name} poster_path = {type === "person" ? item.profile_path : item.poster_path}/>
    })
  return (
